...
class Tokenizer:
    ...
    @classmethod
    def from_data(cls, train_data: list[str], vocab_size: int):
    ...
    def encode(self, text: list[str] | str, add_end_token: bool = True) -> torch.Tensor:
        if isinstance(text, str):
            text += " [END]" if add_end_token else ""
            return torch.tensor(self.tokenizer.encode(text).ids)
        if add_end_token:
            text = [sample + " [END]" for sample in text]
